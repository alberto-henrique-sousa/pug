	...
	protected Control createDialogArea(Composite parent) {
		...
		for (int j = 0; j < table.getColumnCount(); j++) {
			...
			if (this.type > 1 && ((table.getColumn(j).getText().equals("Type") && this.type != 3) || table.getColumn(j).getText().equals("Type Column") || table.getColumn(j).getText().equals("Form Search")
					|| table.getColumn(j).getText().equals("Key")) ) {
				Combo field = new Combo(parent, SWT.READ_ONLY);
				field.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));

				if (table.getColumn(j).getText().equals("Form Search")) {
					field.add("");
					for (String form : CRUDGWTComposite1.listForms) {
						field.add(form);
					}
				} else if (table.getColumn(j).getText().equals("Key")) {
					field.add("true");
					field.add("false");
				} else {
					if (this.type == 2 || this.type == 4) {
						...
						field.add("DoubleBox");
						if (this.type == 4)
							field.add("FileUpload");
						field.add("IntegerBox");
						field.add("ListBox");
						field.add("LongBox"); 
						field.add("MaskDecimalTextBox"); 					 
						field.add("MaskIntegerTextBox"); 					 
						field.add("MaskTextBox"); 					 
						field.add("PasswordTextBox"); 
						field.add("RichTextPug"); 					 
						field.add("TextArea"); 					 
						field.add("TextBox"); 					 
					} else if (this.type == 3) {
						field.add("CheckboxCell");
						field.add("DateCell");
						field.add("DatePickerCell");
						field.add("EditTextCell");
						field.add("NumberCell");
						field.add("SelectionCell");
						field.add("TextCell"); 					  					 
					}
				}
